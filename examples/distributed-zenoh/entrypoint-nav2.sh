...
# Wait for scan data, then set initial pose so AMCL can localize
(
  echo "[nav2-pod] Waiting for /scan data before setting initial pose..."
  for i in $(seq 1 120); do
    if timeout 10 ros2 topic echo /scan --once 2>/dev/null | grep -q "ranges"; then
      echo "[nav2-pod] Scan data detected (attempt ${i}), setting initial pose..."
      sleep 3
